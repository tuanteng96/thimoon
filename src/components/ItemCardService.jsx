...
import { maxBookDate, formatPriceVietnamese, formatDateSv, formatDateNotYYYY, checkAvt2 } from "../constants/format";
            ...
            <div className="cardservice-item__service">
                <div className="cardservice-item__service-img">
                    ...
                    <div className="cardservice-item__service-text">
                        <h4 className="title">{item.OrderItem.ProdTitle} <span>({item.Title})</span></h4>
                        <ul>
                            <li>
                                <span>Giá trị : </span>
                        ...
                        item.Services.map((sub, i) => (
                            <div className="item" key={i}>
                                <div className={"item-box " + this.checkStatus(sub.Status)}>
                                    ...
                                    <span className="hours"><i className="las la-clock"></i>{sub.Minutes}p</span>
                                    <span className="date">{formatDateNotYYYY(sub.BookDate)}</span>
                                    {
                                        sub.Staffs && sub.Staffs.map((user, x) => (
                                            <Link noLinkClass className="link-avatar" popoverOpen={".popover-menu-" + user.ID} key={x}>
                                                <span key={x} className="avatar">
                                                    <img src={checkAvt2(user.Avatar)} alt={user.FullName}/>
                                                    <Popover className={"popover-menu-" + user.ID}>
                                                        <div className="popover-name-nav">Nhân viên thực hiện : <span>{user.FullName}</span></div>
                                                    </Popover>
                                                </span>
                                            </Link>
                                        ))
